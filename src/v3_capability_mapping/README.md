# V3 – Capability-Level Mapping

## Objective

Version 3 extended the Level 1 and Level 2 categorization framework by introducing capability-level mapping.

The purpose was to move beyond individual issue or team-level analysis and provide a broader operational view for capability leaders.

## Why Capability Mapping Was Needed

V1 and V2 classified incidents into increasingly detailed categories.

However, operational improvement often requires visibility across multiple teams that belong to the same broader capability.

For example:

```text
Multiple Small Teams
        ↓
Network Capability

Multiple Small Teams
        ↓
Storage Capability

Multiple Small Teams
        ↓
Windows Capability
